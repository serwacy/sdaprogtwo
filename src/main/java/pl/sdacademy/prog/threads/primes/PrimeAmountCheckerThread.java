...
public class PrimeAmountCheckerThread implements Runnable {
    ...
    public PrimeAmountCheckerThread(final List<Integer> valuesToCheck, final Primes primes) {
    ...
    @Override
    public void run() {
        final long numberOfPrimes = valuesToCheck.stream()
                .filter(value -> this.primes.isPrime(value))
                .count();
        System.out.println("Amount of primes found: " + numberOfPrimes);
    }
}
