...
public class PickEventsStreamProcessor implements StreamProcessor {
    ...
    public PickEventsStreamProcessor(final int maxEventsNum,
                                     final Duration maxProcessingTime,
                                     final PickEventMapper mapper,
    ...
    public void process(@WillNotClose final InputStream source, @WillNotClose final OutputStream sink) throws IOException {
        ...
        final List<PickerStatistics> statistics = pickEventAggregationService.aggregate(pickEvents);
        pickerStatisticsOutputWriter.writeStatistics(statistics,sink);
    }
    private boolean shouldContinueProcessing (final int readEventsNum, final LocalDateTime startingTime){
        return readEventsNum < maxEventsNum && startingTime.plus(maxProcessingTime).isAfter(LocalDateTime.now());
    }
